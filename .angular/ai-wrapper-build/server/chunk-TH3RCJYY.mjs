...
var POST_LOGIN_DEFAULT_ROUTE = "/profile/view-profile";
var LOGIN_ROUTE = "/auth/login";
var PORTFOLIO_URL = "https://arnab-khan.github.io/#/home";

// src/app/core/services/guest.ts
var Guest = class _Guest {
  httpClient = inject(HttpClient);
  platformService = inject(Platform2);
  baseApiUrl = `${environment.rootApiUrl}/guest`;
  guestState = signal({
    isGuest: false,
    guestSessionId: void 0,
    status: "idle",
    completed: false
  }, ...ngDevMode ? [{ debugName: "guestState" }] : []);
  updateGuest(isGuest, options) {
  ...
  createGuest() {
    return this.httpClient.post(`${this.baseApiUrl}/create`, {}).pipe(tap((response) => this.updateGuest(true, { guestSessionId: response.guestSessionId, completed: true })));
  }
  exists() {
    if (!this.platformService.isBrowser()) {
      return of(false);
    }
    this.guestState.update((state) => __spreadProps(__spreadValues({}, state), { status: "loading", completed: false }));
    return this.httpClient.get(`${this.baseApiUrl}/exists`).pipe(tap((response) => this.updateGuest(response.exists, { guestSessionId: response.guestSessionId, updateStatus: false })), map((response) => response.exists), catchError(() => {
  ...
  static \u0275fac = function Guest_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _Guest)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _Guest, factory: _Guest.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(Guest, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var Auth = class _Auth {
  ...
  guest = inject(Guest);
  platformService = inject(Platform2);
  baseApiUrl = `${environment.rootApiUrl}/auth`;
  authState = signal({
    user: null,
    status: "idle",
    completed: false
  ...
  profileState = signal({
    ...
    loading: true,
    loaded: false
  }, ...ngDevMode ? [{ debugName: "profileState" }] : []);
  updateUser(user, options) {
    ...
    if (options?.fetchProfileImage && user?.id) {
      this.injector.get(User).getProfileImage().subscribe();
    }
  }
  signup(body) {
    return this.httpClient.post(`${this.baseApiUrl}/signup`, body).pipe(tap((user) => this.updateUser(user)), finalize(() => this.updateUser(this.authState().user, { updateStatus: false, completed: true, fetchProfileImage: false })));
  }
  login(body) {
    return this.httpClient.post(`${this.baseApiUrl}/login`, body).pipe(tap((user) => this.updateUser(user)), finalize(() => this.updateUser(this.authState().user, { updateStatus: false, completed: true, fetchProfileImage: true })));
  }
  sendForgotPasswordOtp(body) {
    return this.httpClient.post(`${this.baseApiUrl}/forgot-password/send-otp`, body);
  }
  verifyForgotPasswordOtp(body) {
    return this.httpClient.post(`${this.baseApiUrl}/forgot-password/verify-otp`, body);
  }
  resetPassword(body) {
    return this.httpClient.post(`${this.baseApiUrl}/forgot-password/reset`, body);
  }
  me() {
    if (!this.platformService.isBrowser()) {
      return of(null);
    }
    this.authState.update((state) => __spreadProps(__spreadValues({}, state), { status: "loading", completed: false }));
    return this.httpClient.get(`${this.baseApiUrl}/me`).pipe(tap((user) => this.updateUser(user, { updateStatus: false })), catchError(() => {
      this.updateUser(null, { updateStatus: false });
      return of(null);
    }), switchMap((user) => {
      return user ? of(user) : this.guest.exists().pipe(switchMap(() => of(user)));
    }), finalize(() => {
      this.updateUser(this.authState().user, { completed: true, fetchProfileImage: true });
    }));
  }
  logout() {
    return this.httpClient.post(`${this.baseApiUrl}/logout`, {}, { responseType: "text" }).pipe(tap({
      complete: () => {
...
var EMAIL_OTP_SESSION_ERROR_CODES = [
...
var User = class _User {
  ...
  mergeAuthUser(user) {
    this.authService.authState.update((state) => __spreadProps(__spreadValues({}, state), {
      user: state.user ? __spreadValues(__spreadValues({}, state.user), user) : user
    }));
  }
  withProfileImageLoading(request$, options) {
    const showLoader = options?.showLoader;
    if (showLoader) {
      this.authService.profileState.update((state) => __spreadProps(__spreadValues({}, state), { loading: true }));
    }
    return request$.pipe(tap((profileImage) => {
      this.authService.profileState.update((state) => __spreadProps(__spreadValues({}, state), {
        profileImage: state.profileImage ? __spreadValues(__spreadValues({}, state.profileImage), Object.fromEntries(Object.entries(profileImage).filter(([, value]) => value != null))) : profileImage,
        loaded: true
      }));
    }), catchError((error) => {
      ...
      if (showLoader) {
        this.authService.profileState.update((state) => __spreadProps(__spreadValues({}, state), { loading: false }));
      }
    }));
  }
  checkIfUserExists(params) {
    return this.httpClient.get(`${this.baseApiUrl}/public/is-username-exist?${toHttpParams(params)}`);
  }
  getProfileImage(params) {
    return this.withProfileImageLoading(this.httpClient.get(`${this.baseApiUrl}/profile-image${params ? `?${toHttpParams(params)}` : ""}`), { showLoader: !params?.fullImage });
  }
  getFullProfileImage() {
  ...
  updateProfileBasics(body) {
    return this.httpClient.patch(`${this.baseApiUrl}/update-profile-basics`, body).pipe(tap((user) => this.mergeAuthUser(user)));
  }
  updateCredentials(body) {
    return this.httpClient.patch(`${this.baseApiUrl}/update-credentials`, body).pipe(tap((user) => this.mergeAuthUser(user)));
  }
  verifyPassword(body) {
    return this.httpClient.post(`${this.baseApiUrl}/verify-password`, body);
  }
  sendEmailOtp(body) {
    return this.httpClient.post(`${this.baseApiUrl}/email/send-otp`, body).pipe(catchError((error) => {
  ...
  verifyEmailOtp(body) {
    return this.httpClient.post(`${this.baseApiUrl}/email/verify-otp`, body).pipe(tap((user) => this.mergeAuthUser(user)));
  }
  updateProfileImage(profileImage) {
    ...
    return this.withProfileImageLoading(this.httpClient.patch(`${this.baseApiUrl}/update-profile-image`, formData));
  }
  deleteProfileImage() {
    return this.httpClient.delete(`${this.baseApiUrl}/profile-image`).pipe(tap(() => {
      this.authService.profileState.update((state) => __spreadProps(__spreadValues({}, state), {
        profileImage: null,
        loaded: true
      }));
...
//# sourceMappingURL=chunk-TH3RCJYY.mjs.map
